...
- [x] 2. Create shared validation utilities and error classes
...
- [x] 3. Create persistence layer interfaces and in-memory implementation
  - [x] 3.1 Create `src/server/persistence/storage-interface.ts` with `StorageInterface` abstract interface
    ...
    - _Requirements: 6.5_
  - [x] 3.2 Create `src/server/persistence/in-memory-store.ts` implementing `StorageInterface`
  ...
  - [x] 3.3 Create `src/server/persistence/conversation-memory.ts` implementing `ConversationMemory` interface
  ...
  - [x]* 3.4 Write property test: Message persistence round trip (`src/server/persistence/__tests__/in-memory-store.test.ts`)
  ...
  - [x]* 3.5 Write property test: Preference persistence with valid structure (`src/server/persistence/__tests__/in-memory-store.test.ts`)
  ...
  - [x]* 3.6 Write property test: Preference update retains history (`src/server/persistence/__tests__/in-memory-store.test.ts`)
  ...
  - [x]* 3.7 Write property test: Context window stays within token budget (`src/server/persistence/__tests__/conversation-memory.test.ts`)
...
- [x] 4. Checkpoint — System Architect
...
- [x] 5. Create design system tokens and global styles
  - [x] 5.1 Create `src/client/design-system/tokens.ts`
  ...
  - [x] 5.2 Create `src/client/design-system/global-styles.ts`
  ...
  - [x]* 5.3 Write unit tests for design tokens (`src/client/design-system/tokens.test.ts`)
    ...
    - _Requirements: 7.1_
  - [x]* 5.4 Write property test: Design token constraints (`src/client/design-system/tokens.test.ts`)
...
- [x] 6. Checkpoint — UI Designer
  - Ensure all design token tests pass, ask the user if questions arise.

### ⚛️ [Frontend Dev] — Branch: `feat/frontend-valentin-chat-ui`

- [x] 7. Implement state management hooks and context
  - [x] 7.1 Create `src/client/hooks/use-chat-state.ts`
  ...
  - [x] 7.2 Create `src/client/hooks/use-preferences-state.ts`
  ...
  - [x] 7.3 Create `src/client/context/chat-context.tsx` and `src/client/context/preferences-context.tsx`
  ...
  - [x]* 7.4 Write property test: Message submission adds to conversation (`src/client/hooks/__tests__/use-chat-state.test.ts`)
  ...
  - [x]* 7.5 Write property test: Messages display in chronological order (`src/client/hooks/__tests__/use-chat-state.test.ts`)
  ...
  - [x]* 7.6 Write property test: Input cleared after message submission (`src/client/hooks/__tests__/use-chat-state.test.ts`)
...
- [x] 8. Implement WebSocket client hook
  - [x] 8.1 Create `src/client/hooks/use-websocket.ts`
  ...
  - [x]* 8.2 Write unit tests for WebSocket hook (`src/client/hooks/__tests__/use-websocket.test.ts`)
    - Test initial connection status is `'disconnected'`
    - Test reconnection backoff timing
    - Test event dispatching to correct reducers
    - _Requirements: 7.2_

- [x] 9. Implement Chat Panel components
  - [x] 9.1 Create `src/client/components/MessageBubble.tsx`
    ...
    - _Requirements: 5.4_
  - [x] 9.2 Create `src/client/components/MessageHistory.tsx`
    - Render list of `MessageBubble` components from chat state
    - Auto-scroll to bottom on new messages
    - _Requirements: 1.3_
  - [x] 9.3 Create `src/client/components/MessageInput.tsx`
  ...
  - [x] 9.4 Create `src/client/components/TypingIndicator.tsx`
  ...
  - [x] 9.5 Create `src/client/components/ConnectionBanner.tsx`
    - Show banner when `connectionStatus` is `'reconnecting'` or `'disconnected'`
    - Display appropriate message for each status
    - _Requirements: 1.6_
  - [x] 9.6 Create `src/client/components/ChatPanel.tsx`
    - Compose `MessageHistory`, `MessageInput`, `TypingIndicator`, `ConnectionBanner`
    - Wire to `useChatContext` for state and dispatch
    - _Requirements: 1.1, 1.2, 1.3, 1.4, 1.5, 1.6_
  - [x]* 9.7 Write property test: Typing indicator reflects agent processing state (`src/client/components/__tests__/TypingIndicator.test.tsx`)
  ...
  - [x]* 9.8 Write property test: Message styling differs by sender (`src/client/components/__tests__/MessageBubble.test.tsx`)
  ...
  - [x]* 9.9 Write component tests for Chat Panel components (`src/client/components/__tests__/ChatPanel.test.tsx`)
    - Test ChatPanel renders welcome message
    - Test MessageInput clears after submit
    - Test ConnectionBanner shows on disconnect
    - _Requirements: 7.2_

- [x] 10. Implement Profile Dashboard components
  - [x] 10.1 Create `src/client/components/PreferenceCard.tsx`
    - Display preference key, value, confidence badge
    - _Requirements: 4.3_
  - [x] 10.2 Create `src/client/components/PreferenceHighlight.tsx`
  ...
  - [x]* 10.6 Write property test: Preferences grouped by category with correct counts (`src/client/components/__tests__/ProfileDashboard.test.tsx`)
  ...
  - [x]* 10.7 Write property test: Updated preferences show highlight (`src/client/components/__tests__/ProfileDashboard.test.tsx`)
  ...
  - [x]* 10.8 Write component tests for Profile Dashboard (`src/client/components/__tests__/ProfileDashboard.test.tsx`)
...
- [x] 11. Implement App layout and mobile navigation
  - [x] 11.1 Create `src/client/components/MobileNav.tsx`
    - Tab-based toggle between Chat and Profile panels below 768px viewport
    - _Requirements: 5.6_
  - [x] 11.2 Create `src/client/components/AppLayout.tsx`
  ...
  - [x] 11.3 Create `src/client/App.tsx` and `src/client/main.tsx`
    - Wire context providers (ChatContext, PreferencesContext, WebSocketContext)
    - Render AppLayout
    - Add top-level React Error Boundary
    - _Requirements: 4.1, 6.3_
  - [x]* 11.4 Write component tests for layout (`src/client/components/__tests__/AppLayout.test.tsx`)
    - Test dual-panel renders at desktop width
    - Test MobileNav renders at mobile width
    - _Requirements: 7.2_

- [x] 12. Checkpoint — Frontend Dev
...
- [x] 13. Implement API layer (WebSocket gateway and HTTP routes)
  - [x] 13.1 Create `src/server/api/ws-gateway.ts`
  ...
  - [x] 13.2 Create `src/server/api/event-router.ts`
  ...
  - [x] 13.3 Create `src/server/api/http-routes.ts`
    - `GET /health` — health check endpoint
    - `POST /session` — create new session, return `sessionId`
    - `GET /session/:id/preferences` — return preferences for session
    - _Requirements: 6.3_
  - [x]* 13.4 Write unit tests for API layer (`src/server/api/__tests__/event-router.test.ts`)
...
- [x] 14. Implement Agent Orchestrator and Bedrock client
  - [x] 14.1 Create `src/server/agent/bedrock-client.ts`
    ...
    - _Requirements: 6.2_
  - [x] 14.2 Create `src/server/agent/prompts.ts`
  ...
  - [x] 14.3 Create `src/server/agent/agentcore-adapter.ts`
    ...
    - _Requirements: 6.1_
  - [x] 14.4 Create `src/server/agent/agent-orchestrator.ts` implementing `AgentOrchestrator` interface
  ...
  - [x]* 14.5 Write unit tests for Agent Orchestrator (`src/server/agent/__tests__/agent-orchestrator.test.ts`)
...
- [x] 15. Implement Preference Extractor pipeline
  - [x] 15.1 Create `src/server/extraction/category-mapper.ts`
    ...
    - _Requirements: 2.4_
  - [x] 15.2 Create `src/server/extraction/preference-extractor.ts` implementing `PreferenceExtractor` interface
  ...
  - [x]* 15.3 Write unit tests for Preference Extractor (`src/server/extraction/__tests__/preference-extractor.test.ts`)
...
- [x] 16. Implement server entry point and wiring
  - [x] 16.1 Create `src/server/index.ts`
  ...
  - [x]* 16.2 Write unit tests for server wiring (`src/server/__tests__/index.test.ts`)
    - Test server starts without errors with mocked dependencies
    - Test health endpoint returns 200
    - _Requirements: 7.3_

- [x] 17. Checkpoint — Backend Dev
...
- [x] 18. Set up E2E test infrastructure
  - [x] 18.1 Create `playwright.config.ts`
    - Configure Chromium, Firefox, WebKit browsers
    - Set base URL to local dev server
    - Configure screenshot on failure
    - _Requirements: 7.4_
  - [x] 18.2 Create `e2e/fixtures/page-objects.ts`
...
- [x] 19. Write E2E tests for critical user flows
  - [x] 19.1 Create `e2e/tests/onboarding-flow.spec.ts`
  ...
  - [x] 19.2 Create `e2e/tests/responsive-layout.spec.ts`
  ...
  - [x] 19.3 Create `e2e/tests/connection-recovery.spec.ts`
...
- [x] 20. Final Checkpoint — QA Agent
