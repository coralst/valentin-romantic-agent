...
| 🔧 Backend Dev | `agent: backend` | `src/server/`, `agentcore/` | `src/client/`, `src/shared/` |
| 🎨 UI Designer | `agent: design` | `src/client/design-system/`, `docs/design/` | component logic, `src/server/`, shared types |
| 🧪 QA Agent | `agent: qa` | `e2e/`, `playwright.config.ts`, `src/test-setup.ts`, `rehearsal.mjs`, `rehearsal-prod.mjs`, `eval/` | all of `src/` |
| ⚙️ Infra / Workflow | `agent: infra` | `.github/`, `.kiro/`, `.claude/`, `.mcp.json`, `scripts/`, `infra/`, `public/`, `index.html`, `package.json`, `package-lock.json`, `tsconfig.json`, `tsconfig.server.json`, `vite.config.ts`, `Dockerfile`, `.dockerignore`, `.gitignore`, `.env.example`, `README.md`, `CONTRIBUTING.md`, `CLAUDE.md`, `pptx/`, `docs/` | all of `src/` |
