 ...
 *
 * The fixtures deliberately use keys in `food`, `gifts`, `important_dates` and
 * `personality_traits`. Those are the four categories that span several registry
 * fields, so `preference-field-mapper` will not guess a default field for an
 * unrecognized key in them. The single-field categories (`hobbies`, `music`,
 * `travel`, `love_language`) resolve *any* key to their one field, which makes
 * them useless for exercising the unmapped path — an earlier version of this file
 * used `hobbies:collections` and started passing vacuously once that fallback
 * landed. Each test still guards its own premise with `resolveField`.
 */
describe('AlsoMentioned — rescuing unmapped extraction output', () => {
  it('renders a preference that resolves to no registry field', () => {
    ...
    expect(screen.getByText('anything crunchy')).toBeInTheDocument();
  });
  ...
  it('reuses CategoryGroup and PreferenceCard, grouped by category', () => {
    render(
      <AlsoMentioned
        preferences={[
          preference({ id: 'a', category: 'food', key: 'texture' }),
          preference({ id: 'b', category: 'gifts', key: 'wrapping style', value: 'brown paper' }),
        ]}
      />,
    );
  ...
  it('counts every rescued preference in the head pill', () => {
    render(
      <AlsoMentioned
        preferences={[
          preference({ id: 'a', key: 'texture' }),
          preference({ id: 'b', key: 'plating style', value: 'family platters' }),
        ]}
      />,
    );
...
describe('groupUnmappedPreferences', () => {
  it('orders buckets canonically rather than by arrival', () => {
    const groups = groupUnmappedPreferences([
      preference({ id: 'a', category: 'gifts', key: 'wrapping style' }),
      preference({ id: 'b', category: 'food', key: 'texture' }),
    ]);
    // `food` precedes `gifts` in PREFERENCE_CATEGORIES, so the card's sections
    // do not reshuffle as new extractions arrive.
    expect(groups.map((group) => group.category)).toEqual(['food', 'gifts']);
  });
