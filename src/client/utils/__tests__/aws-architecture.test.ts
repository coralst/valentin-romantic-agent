...
describe('engine membership', () => {
  ...
  it('never maps a node onto one the target engine does not have, bar the two with no counterpart', () => {
    ...
    // Engine A does its own memory and calls its tools in-process, so these two
    // genuinely have nothing to translate to. They are returned unchanged and the
    // view shades them — which is honest — rather than being mapped to a
    // resource engine A does not have.
